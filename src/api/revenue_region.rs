...
async fn revenue_by_region_route(
    db: web::Data<PgPool>,
    ...
    let result = sqlx::query_as::<_, (String, BigDecimal)>(
        "SELECT regions.name AS name, 
         ...
         FROM orders
         JOIN order_regions ON orders.id = order_regions.order_id
         JOIN regions ON order_regions.region_id = regions.id
    ...
    match result {
        ...
        Err(err) => {
            error!("Failed to fetch revenue by region: {}", err);
            HttpResponse::InternalServerError().body("Failed to fetch revenue by region")
        }
    }
}
