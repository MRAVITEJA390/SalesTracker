...
async fn revenue_by_category_route(
    db: web::Data<PgPool>,
    ...
    let result = sqlx::query_as::<_, (String, BigDecimal)>(
        r#"SELECT products.category AS name, 
    ...
    match result {
        ...
        Err(err) => {
            error!("Failed to fetch revenue by category: {}", err);
            HttpResponse::InternalServerError().body("Failed to fetch revenue by category")
        }
    }
}
